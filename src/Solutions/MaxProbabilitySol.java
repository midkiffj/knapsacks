...
public class MaxProbabilitySol extends ProblemSol {
	...
	public ProblemSol crossover(ProblemSol ps2) {
		...
		if (newTotalU < mp.getT()) {
			System.err.println("Infeasible generated by adding ratio");
			x = tempX;
			r = tempR;
			newTotalA = 0;
			newTotalU = 0;
		...
		//		int count = 0;
		//		while (newTotalU < mp.getT() && feasible) {
		//			int maxU = mp.maxURatio(r);
		//			int minU = mp.minURatio(x);
		//			if (newTotalA + mp.getA(maxU) - mp.getA(minU) <= mp.getB()) {
		//				x.add(maxU);
		//				r.remove(Integer.valueOf(maxU));
		//				x.remove(Integer.valueOf(minU));
		//				r.add(minU);
		//				newTotalA = mp.addA(maxU, mp.removeA(minU, newTotalA));
		//				newTotalU = mp.addU(maxU, mp.removeU(minU, newTotalU));
		//				newXVals[maxU] = true;
		//				newXVals[minU] = false;
		//			} else {
		//				int uchange = 0;
		//				int uj = -1;
		//				for (Integer j: x) {
		//					if (newTotalA + mp.getA(maxU) - mp.getA(j) <= mp.getB()) {
		//						int jUchange = mp.getU(maxU) - mp.getU(j);
		//						if (jUchange > uchange) {
		//							uchange = jUchange;
		//							uj = j;
		//						}
		...
		//					x.add(maxU);
		//					r.remove(Integer.valueOf(maxU));
		//					x.remove(Integer.valueOf(uj));
		//					r.add(uj);
		//					newTotalA = mp.addA(maxU, mp.removeA(uj, newTotalA));
		//					newTotalU = mp.addU(maxU, mp.removeU(uj, newTotalU));
		//					newXVals[maxU] = true;
		//					newXVals[uj] = false;
		//				} else {
		//					feasible = false;
